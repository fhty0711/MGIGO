...
# ======================================================================
# 新增辅助函数：用于在重置步缩放精度矩阵的特征值以鼓励探索
# ======================================================================
@jit
def _explore_precision_matrix(S, nu):
    
    eigvals, eigvecs = jnp.linalg.eigh(S)
    # 缩小精度矩阵特征值 = 放大协方差 = 鼓励探索
    eigvals_new = eigvals / nu
    # 依然需要保证不低于 MIN_EIG 边界
    eigvals_new = jnp.maximum(eigvals_new, MIN_EIG)
    return eigvecs @ (eigvals_new[:, None] * eigvecs.T)
...
def _step_fn(state, iter_data, M, K, B, B0, dt, dims_arr, T_0, nu_r, fitness_fn, v_reset, context):
    mu, S, v, t = state
    key, _ = iter_data
    
    # ------------------------------------------------------------------
    # 💡 核心修改点：实现 Algorithm 5 的高效重置与探索步
    # ------------------------------------------------------------------
    # 当 t > 0 且 t % T_0 == 0 时，触发重置
    is_reset_turn = (t > 0) & ((t % T_0) == 0)
    
    # 1. 重置权重对应的隐变量 v
    v = jnp.where(is_reset_turn, v_reset, v)
    
    # 2. 探索机制：缩放精度矩阵 S 的特征值 (利用 vmap 作用于所有块和分量)
    #S_explored = vmap(vmap(lambda mat: _explore_precision_matrix(mat, nu_r)))(S)
    #S = jnp.where(is_reset_turn, S_explored, S)

    # 另一种选择：重置的时候直接让方差就是1.0I. 这相当于完全放弃当前的精度信息，鼓励全局探索。
    S = jnp.where(is_reset_turn, jnp.eye(S.shape[-1]), S)
    # ------------------------------------------------------------------
...
@functools.partial(jit, static_argnums=(1, 3, 4, 5, 7, 10))
def mmog_igo_optimizer_mpc(
    key, T, dt, M, K, B, B0, dims, T_0, nu_r,
    fitness_fn_total, initial_mu_k, initial_L_inv_k, initial_v_k, context
):
    """
    增加了 nu_r 参数：用于控制重置步的探索力度 (协方差放大倍数)。
    如果不需要特征值缩放探索，将 nu_r 设为 1.0 即可。
    """
    ...
    loop_fn = functools.partial(
        ...
        dims_arr=dims_array, T_0=T_0, nu_r=nu_r,
