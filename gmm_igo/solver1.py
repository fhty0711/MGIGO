...
@functools.partial(jax.jit, static_argnames=('B', 'B_0', 'K', 'delta_t', 'fitness_fn'))
def _iteration_step(state, key_input, B, B_0, K, delta_t, fitness_fn, context):
    ...
    # --- 3. 更新混合权重 [调整后的顺序：先计算 pi 的更新] (对应算法 Step 14-16) ---
    # 我们调用 vmap 化的单组件更新函数，它会返回各个组件的 v_update_sum
    ...
    v_k_t_plus_1 = jnp.clip(v_k_t_plus_1, a_max=70.0)
    
    # --- 4. 更新 Gaussian 分量参数 (对应算法 Step 18-23) ---
    # 注意：mu_k_t_plus_1 和 L_inv_k_t_plus_1 已经在上面的 _vmap_update_step_k_l 中算好了
    # 它们内部严格遵循了先更新 S，再用新 S 更新 mu 的逻辑。
