...
def _step_fn_rne(state, iter_data, M_agent, K, B, B0, dt, dims_arr, T_0, fitness_fns, v_reset, context, M_inner):
    ...
    # 抽取计算期望 Cost 的核心逻辑，同时传出每个样本在对方环境里的平均真实适应度
    def evaluate_expected_cost(agent_idx):
        zb_candidates = samples_B[agent_idx]
        fitness_fn = fitness_fns[agent_idx]
        ...
        return elite_weights, jnp.mean(f_hat) # 额外返回当前博弈下该 agent 的平均估计代价

    # 展开计算
    eval_results = [evaluate_expected_cost(idx) for idx in range(M_agent)]
    w_hat_m = jnp.stack([res[0] for res in eval_results])
    mean_fitness_m = jnp.stack([res[1] for res in eval_results]) # Shape: (M_agent,)
    ...
    # 【高效记录核心】：将每一轮迭代产生的数据直接打包放入 metrics 字典返回
    # 这样 lax.scan 会自动帮我们在 T 维度上堆叠（vmap 级的高效内存连续存储）
    metrics = {
        "mu": mu,                         # 当前轮迭代各模态轨迹均值，Shape: (M_agent, K, HORIZON)
        "pi": pi_all,                     # 当前轮迭代各模态混合概率，Shape: (M_agent, K)
        "mean_fitness": mean_fitness_m     # 当前轮每个 Agent 针对对端估计出的平均期望博弈 Cost，Shape: (M_agent,)
    }
...
def mmog_igo_rne_solver(key, T, dt, M_agent, K, B, B0, dims, T_0, 
    ...
    """
    更新后的求解器：现在它会返回第四个参数 `metrics_history`，供外部追溯整个收敛过程。
    """
    ...
    # 关键修改：lax.scan 的第二个位置捕获所有迭代轮次的指标集合
