# gmm_igo/MPCsolver_Hetero.py - 支持异构维度的 M-MoG IGO 优化器核心 (修复 TypeError)
...
def _sample_from_component_l(idx, key_sample, mu_k_all, L_inv_k_all):
    ...
    # 计算协方差 Sigma = S^{-1} 的 Cholesky 因子 L_Sigma
    # 使用 jnp.linalg.solve 更稳定
...
# 修正：移除 initial_L_inv_k_static 参数，因为它在 partial 绑定时被错误地传递了
def _parallel_step_with_t0_restart(
    carry, t, 
    M, K, B, B0, dt, dims_array, D_max, T_0, 
    initial_v_k_static, # 仅保留需要重置的权重
    ...
    # 🌟 逻辑：只重置 v_M (权重)，保持 mu_M 和 L_inv_M 不变
    # 重置为初始状态 (即零向量，对应均匀权重)
    new_v_M = jnp.where(do_restart, initial_v_k_static, v_M)
    
    # 构造当前状态 (mu_M 和 L_inv_M 使用当前值，v_M 使用重置或当前值)
    current_carry = (mu_M, L_inv_M, new_v_M) 
...
def _compute_pi(v_m):
    ...
    pi_K = 1 / (1 + jnp.sum(pi_pre))
    # FIX: 明确将 pi_K 转换为 (1,) 数组
    return jnp.concatenate([pi_pre * pi_K, jnp.expand_dims(pi_K, axis=0)])
...
def mmog_igo_optimizer_mpc_impl(
    ...
    # 初始重启状态 V_k (全部 M 个 MoG 的 v_k 都重置为零向量)
    # 对应于权重 pi_k 的均匀分布
    initial_v_k_static = jnp.zeros((M, K)) 
    
    t_iterations = jnp.arange(T)
    
    # 修复：移除 initial_L_inv_k_static 的绑定
    bound_iteration_step = functools.partial(
        ...
        D_max=D_max, 
        T_0=T_0,
        initial_v_k_static=initial_v_k_static, # 仅绑定 V
