...
方案 A（先加总再 Soft）： 只看总穿透量。200 点擦边（2.0m）和 1 点深撞（2.0m）在 Soft 眼里完全等价。

方案 B（每步 Tunable 再加总）： 总穿透相同（2.0m），但 1 点深撞（0.771）比 200 点擦边（0.926）代价更低。因为 Tunable 把大穿透压缩了——2.0m 的单点贡献被 $\sigma$ 从 2.0 压到 0.74，而 200 个 0.01m 的贡献几乎是线性累加。

方案	200点擦边 2.0m	1点深撞 2.0m	谁更差

A (sum→Soft)	0.843	0.843	一样

B (每步Tun→sum)	0.926	0.771	多点擦边更差

这就有意思了——方案 B 实际上表达了"宁可偶尔深撞，也不要全程擦边"的语义。 因为大穿透被 $\sigma$ 压扁，而小穿透的累加不会被压缩。

反过来，如果你认为"单点深撞比多点轻擦更危险"，加回 1.5 floor 或者用更大的 $\beta$（让 Tunable 在更小穿透时就饱和）就能翻转这个排序。

所以本质上你手里有三个旋钮来控制"多点轻擦 vs 单点深撞"的权衡：

旋钮	作用
1.5 floor	区分"擦边"和"安全"，防止零穿透时无信号

加总策略（先加 vs 先Tunable再加）	控制累加效应是线性的还是被压缩的

β（Tunable 的锐度）	控制单点大穿透被压扁的程度
