...
**关键设计**：树是静态索引——不是在搜索中动态扩展的。所有路径在建模时已存在。

```
时域索引树（H=6, 2台机组, 每台可选 ON/OFF）

深度 = 时域内的拓扑变更事件数（不是 H × N_gen）

Timestep:  t      t+1    t+2    t+3    t+4    t+5
           ─────────────────────────────────────────
Gen 0:     ON ──── ON ──── ON ──── ON ──── ON ──── ON   (baseload, 不变)
Gen 1:     OFF ─── OFF ─── ON?──── ON ──── ON ──── OFF? (peak, 最多2次变更)

变更事件:
  Event 1: Gen 1 在 t+2 开机 → 分支: ON at t+2 / 不开
  Event 2: Gen 1 在 t+5 关机 → 分支: OFF at t+5 / 继续开
```

**树的每个路径 = 时域内的完整拓扑序列**。叶节点包含 $\mathbf{s}_{t:t+H}$ 的全部信息。

**为什么时域内的拓扑变更事件数远小于 $H \times N$**：
- 最小启停时间约束：一台机不会在 2-3 个时步内反复开关
- 短时域（MPC 典型 $H \leq 12$）：在 12 个时步内，大多数机组最多变更 1-2 次状态
- 宏决策压缩：不是逐时步决策 ON/OFF，而是决策"变更点"（changepoint）：从何时开始 ON，到何时转为 OFF
...
**我们的"树"是什么**：
- 一旦建模完成，树是**静态且完整的**——所有从根到叶的路径（离散变量组合）**从一开始就已经存在**
- 每条路径是一个完整的离散决策组合，对应一个确定的结构 $s$。100 台机组的 UC 问题 → 树的叶节点 = $2^{100}$ 个启停组合（当然实际上通过分块和宏决策压缩到可控范围）
